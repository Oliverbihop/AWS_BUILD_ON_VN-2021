...
class Unet():
    def __init__(self):
        ...
        self.regression = pickle.load(open('./server/deep/finalized_model.sav', 'rb'))
    ...
    def predictPrice(self, area, obj):
        X = np.array([area, obj]).reshape(1,-1)
        price = self.regression.predict(X)
        return price[0]
    ...
    def predict(self, hashImage):
        ...
        price = self.predictPrice(area, len(contours))
        return {
